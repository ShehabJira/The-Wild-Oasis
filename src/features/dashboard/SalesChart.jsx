...
// ]; // we will compute a datastructure like this one.
...
function SalesChart({ bookings, numDays }) {
	...
	const data = allDates.map((date) => {
		return {
			...
			totalSales: bookings
				.filter((booking) => isSameDay(date, new Date(booking.created_at))) // we need to get all the bookings for the current date //Note! new Date() is used because this library always needs to work with JS date objects.
				.reduce((acc, cur) => acc + cur.totalPrice, 0), // and then compute total sales of these bookings of that day.
		...
		<StyledSalesChart>
			...
			<ResponsiveContainer width="100%" height={300}>
				<AreaChart data={data}>
					...
					<Area
						dataKey="totalSales" // specify what data this area should be based on.
						type="monotone"
						stroke={colors.totalSales.stroke}
						fill={colors.totalSales.fill}
						unit="$"
						name="Total sales" // if not provided it will take the dataKey as the name of this area.
						strokeWidth={2} // the top border width of the area.
					/>
					<Area
						dataKey="extrasSales"
						type="monotone"
